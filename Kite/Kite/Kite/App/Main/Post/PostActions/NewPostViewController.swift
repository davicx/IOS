...
class NewPostViewController: UIViewController {
    ...
    private let captionInput = componentFunctions.createItemDescriptionInput()
    private let addPhotoButton = componentFunctions.createAddPhotoButton()
    private let photoPreviewImageView = UIImageView()
    private let submitPostButton = componentFunctions.createSubmitItemButton()
    ...
    func setupNewPostLayout() {
        ...
        submitPostButton.addTarget(self, action: #selector(submitPostTapped), for: .touchUpInside)
        ...
        NSLayoutConstraint.activate([
            ...
            captionInput.topAnchor.constraint(equalTo: titleLabel.bottomAnchor, constant: 20),
            captionInput.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            captionInput.widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: 0.8),
            captionInput.heightAnchor.constraint(equalToConstant: 120),

            addPhotoButton.topAnchor.constraint(equalTo: captionInput.bottomAnchor, constant: 20),
            ...
            submitPostButton.topAnchor.constraint(equalTo: photoPreviewImageView.bottomAnchor, constant: 20),
            submitPostButton.centerXAnchor.constraint(equalTo: view.centerXAnchor),
            submitPostButton.widthAnchor.constraint(equalTo: view.widthAnchor, multiplier: 0.8),
            submitPostButton.heightAnchor.constraint(equalToConstant: 32)
        ])
    }
    ...
    @objc private func submitPostTapped() {
        ...
        let postTo = "\(groupID)"
        let postCaption = captionInput.text?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        ...
        Task {
            let success = await PostLogic.shared.createPhotoPost(
                postImage: postImage,
                postFrom: postFrom,
                postTo: postTo,
            ...
            DispatchQueue.main.async {
                ...
                if success {
                    print("Photo post created successfully!")
                    self.dismiss(animated: true)
                } else {
                    print("Failed to create photo post")
                }
            }
        }
    }
