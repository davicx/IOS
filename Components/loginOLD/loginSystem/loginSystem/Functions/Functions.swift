...
func registerUser(userName: String, fullName: String, email: String, password: String, completionHandler:@escaping (RegisterUserModel, Error?)->Void ) {
    let parameters = [
        "user_name": userName,
        "full_name": fullName,
        "email": email,
    ...
    session.dataTask(with: request) { (data, response, error) in
        if let data = data {
            do {
                ...
                let registerUserOutcome = try JSONDecoder().decode(RegisterUserModel.self, from: data)
