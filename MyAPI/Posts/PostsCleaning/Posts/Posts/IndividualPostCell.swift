...
class IndividualPostCell: UITableViewCell {
    ...
    let layoutManager = IndividualPostLayoutManager()
    ...
    func updatePost(with currentImage: UIImage, postCaption: String) {
        ...
        // Update image height dynamically
        postImageHeightConstraint?.constant = imageHeight
        
        layoutManager.postImageView.image = currentImage
        layoutIfNeeded()
    }
}
...
class IndividualPostCell: UITableViewCell {
    ...
    private let layoutManager = IndividualPostLayoutManager()
